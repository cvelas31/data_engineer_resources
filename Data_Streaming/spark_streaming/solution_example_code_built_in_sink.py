...
def built_in_sink_exercise():
    ...
    # Set the schema of the JSON data
    # Treat a sequence of files as a stream by picking one file at a time
    streaming_input_df = spark.readStream.schema(jsonSchema)\
                    .option("maxFilesPerTrigger", 1).json('data.txt')
    
    
    streaming_count_df = streaming_input_df.groupBy(
            streaming_input_df.status,
            window(streaming_input_df.timestamp, "1 hour")).count()
    ...
    query = streaming_count_df.writeStream.format("console")\
                        .queryName("counts").outputMode("complete").start()
